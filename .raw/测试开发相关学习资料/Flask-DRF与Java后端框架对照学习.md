...
> **定位**：面向已掌握 Flask / DRF 的测试开发工程师，用「已知映射未知」的方式快速建立 Java 后端框架知识面，应付面试中的框架类问题。
> **不需要会写 Java 代码**，重点是「听得懂、说得清、能对上号」。

---

## 一、先建立整体认知：Java 后端世界的一张地图

面试中听到 Java 后端，90% 的场景说的是 **Spring 全家桶**。先记住这条演进线：
...
> ⚠️ **常见误区提醒**：Flask 和 Spring Boot 并不严格对等。Spring Boot 的定位其实更像「Django + DRF」这种全家桶——它把 Web 层、配置管理、数据库连接、内嵌服务器全部打包好了。面试时可以说：「Flask 是微框架，Spring Boot 则是全家桶式的一站式框架，定位上更接近 Django。」
...
> 💡 面试点到为止即可：「DRF 用 Serializer 一个类同时管序列化和校验；Java 这边拆成了两个东西——Jackson 管序列化（自动的），Bean Validation 注解管校验。」

### 2.3 分层架构（面试必考）

Java 后端非常讲究**严格分层**，这是和 Flask 项目最大的风格差异，也是面试官最爱问的：
...
**为什么要背下来**：你在 Flask 项目里其实做过同样的事（视图 → 业务函数 → ORM），只是 Flask 不强制。面试可以说：「Java 把 Flask 里的最佳实践变成了强制分层约定，Controller 必须薄、Service 承载业务、Mapper 只管 SQL。我在 Python 项目里做接口自动化时，定位 bug 也是按这个分层思路排查的——先确认是接口层参数问题、业务逻辑问题还是 SQL 问题。」

### 2.4 依赖注入（IoC/DI）—— Java 特有的核心概念

这是 Python 程序员理解 Spring 的唯一「新」概念，务必理解：

- **IoC（控制反转，Inversion of Control）**：对象不自己 `new` 依赖，而是由 Spring 容器统一创建和管理对象（这些被管理的对象叫 **Bean**）。
- **DI（依赖注入，Dependency Injection）**：容器把依赖自动「注入」给需要它的对象。

**用你熟悉的东西类比**：

> 你在 pytest 里用过 **fixture** 吧？`def test_x(db_conn)` —— 测试函数不自己创建 `db_conn`，而是声明「我需要它」，pytest 框架负责创建并注入进来。**Spring 的依赖注入本质上就是一个贯穿整个应用的巨型 fixture 系统**：Controller 声明「我需要 UserService」，Spring 容器负责创建并注入。
...
- `@Component` / `@Service` / `@Repository` / `@RestController`：把类标记为 Bean，交给容器管理（≈ 告诉 pytest「这是一个 fixture」）。
- `@Autowired`：自动注入（老写法，现在推荐构造器注入）。

### 2.5 AOP（面向切面编程）

- **AOP（Aspect-Oriented Programming）**：把「横切关注点」（日志、事务、权限、耗时统计）从业务代码中剥离，统一在「切面」里处理。
- **Python 类比**：你已经见过这个思想——**装饰器**和 **Flask 的 `before_request` 钩子**、**Django 中间件**就是穷人版 AOP。`@login_required` 套在函数上，就是「在方法执行前织入一段权限校验逻辑」，这正是 AOP 的典型场景。
- Spring 中典型应用：`@Transactional`（声明式事务，一个注解搞定事务提交/回滚）、`@Async`、自定义日志切面。

### 2.6 中间件 / 拦截器 / 过滤器

请求处理链上的「关卡」，两边都有，Java 分得更细：

| 层级 | Python | Java |
|------|--------|------|
| 请求前后统一处理 | Flask `before_request` / `after_request`、Django 中间件 | **Filter（过滤器）**：Servlet 层面，最先执行 |
| Controller 前后处理 | DRF 的 permission_classes | **Interceptor（拦截器，HandlerInterceptor）**：进入 Controller 前后 |
...
> 对测试工程师的实际意义：Java 项目改测试环境地址，通常就是改 `application-test.yml` 里的 `spring.datasource.url`，或者用启动参数 `--spring.profiles.active=test` 切换——做接口自动化联调时经常要让开发给你这个文件。
...
> 💡 「MyBatis vs JPA」是经典面试题：JPA 像 Django ORM（全自动，但复杂 SQL 不灵活）；MyBatis 手写 SQL（可控性强，适合国内复杂报表业务，所以国内公司绝大多数用 MyBatis / MyBatis-Plus）。作为测试，看懂 MyBatis 的 XML 就能直接知道接口背后跑了什么 SQL，对造测试数据和排查 bug 很有帮助。
...
> 部署相关面试点：「Spring Boot 为什么流行？」—— 内嵌容器 + 自动配置 +  starters 依赖管理，让 Java 项目从「配置一大堆 XML、部署到外部 Tomcat」变成「一个 JAR 包 java -jar 就跑起来」，这是它能成为微服务标配的原因。
...
|---------|--------|------|-----------|
| 单元测试 | pytest（fixture / parametrize / mock） | JUnit 5（@Test / @ParameterizedTest）+ Mockito（mock） | 概念完全互通，注解 vs 装饰器 |
...
3. 如果面试机会允许，把「我读得懂 Java 后端代码，能结合实现设计接口用例」作为差异化优势主动讲——这是纯 Python 测试工程师不具备的。
4. 想再加深印象，可以在公司随便找一个 Java 项目的 Controller + Mapper.xml 读 10 分钟，比看任何教程都快。
